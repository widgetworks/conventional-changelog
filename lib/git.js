...
function latestTag(done) {
  ...
  // 2014-06-02 Coridyn:
  // One-liner in Windows returning invalid value: git describe --tags `git rev-list --tags --max-count=1`
  // Split into two calls to get it working.
  cp.exec("git rev-list --tags --max-count=1", function(err, stdout, stderr) {
    if (err) {
      getFirstCommit(done);
    } else {
      cp.exec("git describe --tags " + String(stdout).trim(), function(err, stdout, stderr) {
        if (err) {
          getFirstCommit(done);
        } else {
